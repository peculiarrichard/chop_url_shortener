...
const ShortLinks = () => {
      ...
      <section className="w-full ml-[15rem] box-border p-4 text-[#2e4457]">
        <div className="flex justify-between border-b py-4">
          <div className="flex gap-4 justify-center items-center">
            ...
            <p className="font-semibold">Create short links with auto backhalves</p>
          </div>
          <button className="flex gap-2 items-center p-2 border-2 rounded-lg text-[0.8rem] font-semibold hover:bg-[#2e4457] hover:text-white hover:border-none">
  ...
  <path d="M2 0a2 2 0 0 0-2 2v12.793a.5.5 0 0 0 .854.353l2.853-2.853A1 1 0 0 1 4.414 12H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
        ...
        <div className="flex flex-col items-center border-2 rounded-lg h-full p-3 mt-3">
          <input
          type="text"
          placeholder="Title"
          value={urlName}
          ...
          required
          className="w-[50%] text-center p-4 border border-[#2e4457] rounded"
        />
        <input
          type="url"
          ...
          required
          className="w-[50%] text-center p-4 border border-[#2e4457] rounded mt-4"
        />
        <button onClick={handleShorten} disabled={loading} className="p-3 bg-[#005AE2] rounded-lg mt-2 text-white">Chop URL
        {loading? <Loading /> : null}
        </button>
        <p className="mt-2 text-[0.7rem]">By clicking Chop URL, I agree with the terms of service, privacy policy, and use of cookies.</p>
      </div>
      {shortenedUrl && (
          <div>
            <h1>Title: {urlName}</h1>
        ...
        <div className="mt-6">
          {links.length === 0 ? (
            ...
            <ul className=" w-full  ">
              {links.map((link: any) => (
                <li key={link.shortUrl} className="justify-between mt-3 flex border-2 shadow-lg p-3 rounded">
                  <div className="flex flex-col space-y-2">
                  <h1 className="text-xl font-bold">{link.name}</h1>
                  <p className="overflow-x-hidden font-bold">Long Url: {link.longUrl} </p>
                  <p className="font-bold">Short URL: {link.shortUrl}</p>
                  <p className="text-[0.8rem]">Created at: {link.createdAt._seconds} </p>
